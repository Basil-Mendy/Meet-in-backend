...
class WalletTransferTests(TestCase):
    def setUp(self):
        ...
        self.user1_wallet, _ = PaymentUserWallet.objects.get_or_create(user=self.user1)
        self.user2_wallet, _ = PaymentUserWallet.objects.get_or_create(user=self.user2)
        self.user1_wallet.balance = Decimal("1000.00")
        self.user2_wallet.balance = Decimal("100.00")
        self.user1_wallet.save(update_fields=["balance"])
        self.user2_wallet.save(update_fields=["balance"])
        self.forum1_wallet, _ = ForumWallet.objects.get_or_create(forum=self.forum1)
        self.forum2_wallet, _ = ForumWallet.objects.get_or_create(forum=self.forum2)
        self.forum1_wallet.balance = Decimal("500.00")
        self.forum2_wallet.balance = Decimal("250.00")
        self.forum1_wallet.save(update_fields=["balance"])
        self.forum2_wallet.save(update_fields=["balance"])
...
class DisbursementApprovalTests(TestCase):
    def setUp(self):
        ...
        self.forum_wallet, _ = ForumWallet.objects.get_or_create(forum=self.forum)
        self.forum_wallet.balance = Decimal("10000.00")
        self.forum_wallet.save(update_fields=["balance"])
        ...
        self.member_wallet, _ = PaymentUserWallet.objects.get_or_create(user=self.member)
        self.member_wallet.balance = Decimal("0.00")
        self.member_wallet.save(update_fields=["balance"])
        self.client = APIClient()
